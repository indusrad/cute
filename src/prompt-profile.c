...
gboolean
prompt_profile_apply (PromptProfile     *self,
                      ...
                      GError           **error)
{
  ...
  if (prompt_profile_get_use_custom_command (self))
    {
      custom_command = prompt_profile_dup_custom_command (self);

      if (!g_shell_parse_argv (custom_command, NULL, &argv, error))
        return FALSE;

      prompt_run_context_append_args (run_context, (const char * const *)argv);
      arg0 = argv[0];
    }
  else
    {
      arg0 = default_shell;
      prompt_run_context_append_argv (run_context, arg0);
    }

  if (prompt_profile_get_login_shell (self) &&
      prompt_shell_supports_dash_l (arg0))
    prompt_run_context_append_argv (run_context, "-l");

  if (current_directory_uri != NULL)
    last_directory = g_file_new_for_uri (current_directory_uri);

  preserve_directory = prompt_profile_get_preserve_directory (self);
