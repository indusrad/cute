...
static const PromptPaletteData palettes[] = {
  {
    .id = "gnome",
    .name = N_("GNOME"),
    .light = {
...
const GdkRGBA *
prompt_palette_get_background (PromptPalette *self,
                               gboolean       dark)
{
...
const GdkRGBA *
prompt_palette_get_foreground (PromptPalette *self,
                               gboolean       dark)
{
  g_return_val_if_fail (PROMPT_IS_PALETTE (self), NULL);
...
const GdkRGBA *
prompt_palette_get_indexed_colors (PromptPalette *self,
                                   guint         *n_colors)
{
  g_return_val_if_fail (PROMPT_IS_PALETTE (self), NULL);
...
prompt_palette_list_model_get_default (void)
{
  ...
  if (instance == NULL)
    {
      instance = g_list_store_new (PROMPT_TYPE_PREFERENCES_LIST_ITEM);
      g_object_add_weak_pointer (G_OBJECT (instance), (gpointer *)&instance);

      for (guint i = 0; i < G_N_ELEMENTS (palettes); i++)
        {
          g_autoptr(GVariant) value = g_variant_take_ref (g_variant_new_string (palettes[i].id));
          g_autoptr(PromptPreferencesListItem) item = g_object_new (PROMPT_TYPE_PREFERENCES_LIST_ITEM,
