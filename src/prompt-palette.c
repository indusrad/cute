  ...
  if (instance == NULL)
    {
      ...
      for (guint i = 0; i < G_N_ELEMENTS (prompt_palettes_inline); i++)
        {
          g_autoptr(GVariant) value = g_variant_take_ref (g_variant_new_string (prompt_palettes_inline[i].id));
          g_autoptr(PromptPreferencesListItem) item = g_object_new (PROMPT_TYPE_PREFERENCES_LIST_ITEM,
                                                                     "title", g_dgettext (GETTEXT_PACKAGE, prompt_palettes_inline[i].name),
                                                                     "value", value,
                                                                     NULL);
