...
enum {
  PROP_0,
  PROP_SETTINGS,
#define CAPSULE_SHORTCUT_DEFINE(NAME, name) PROP_##NAME,
# include "capsule-shortcuts.defs"
#undef CAPSULE_SHORTCUT_DEFINE
  N_PROPS
};

G_DEFINE_FINAL_TYPE (CapsuleShortcuts, capsule_shortcuts, G_TYPE_OBJECT)
...
static void
capsule_shortcuts_settings_changed_cb (CapsuleShortcuts *self,
                                       const char       *key,
                                       GSettings        *settings)
{
  g_assert (CAPSULE_IS_SHORTCUTS (self));
  g_assert (key != NULL);
...
static void
capsule_shortcuts_constructed (GObject *object)
{
  CapsuleShortcuts *self = (CapsuleShortcuts *)object;

  G_OBJECT_CLASS (capsule_shortcuts_parent_class)->constructed (object);
  ...
  g_signal_connect_object (self->settings,
                           "changed",
                           G_CALLBACK (capsule_shortcuts_settings_changed_cb),
                           self,
                           G_CONNECT_SWAPPED);
...
static void
capsule_shortcuts_dispose (GObject *object)
{
  CapsuleShortcuts *self = (CapsuleShortcuts *)object;

  g_clear_object (&self->settings);

  G_OBJECT_CLASS (capsule_shortcuts_parent_class)->dispose (object);
}

static void
capsule_shortcuts_get_property (GObject    *object,
                                guint       prop_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
  CapsuleShortcuts *self = CAPSULE_SHORTCUTS (object);
...
#define CAPSULE_SHORTCUT_DEFINE(NAME, name) \
    case PROP_##NAME: \
...
static void
capsule_shortcuts_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
...
static void
capsule_shortcuts_class_init (CapsuleShortcutsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = capsule_shortcuts_constructed;
  object_class->dispose = capsule_shortcuts_dispose;
  object_class->get_property = capsule_shortcuts_get_property;
  object_class->set_property = capsule_shortcuts_set_property;
...
static void
capsule_shortcuts_init (CapsuleShortcuts *self)
{
}

CapsuleShortcuts *
capsule_shortcuts_new (GSettings *settings)
{
  g_return_val_if_fail (!settings || G_IS_SETTINGS (settings), NULL);

  return g_object_new (CAPSULE_TYPE_SHORTCUTS,
                       "settings", settings,
                       NULL);
}
...
static GMenuItem *
capsule_shortcuts_copy_menu_item (GMenu *menu,
                                  guint  index)
{
  GMenuItem *copy;
...
static void
capsule_shortcuts_replace_key (GMenu      *menu,
                               guint       index,
                               const char *key,
                               const char *value)
{
  ...
  copy = capsule_shortcuts_copy_menu_item (menu, index);
...
/**
 * capsule_shortcuts_update_menu:
 * @self: a #CapsuleShortcuts
 * @menu: a #GMenu
 *
 ...
 */
void
capsule_shortcuts_update_menu (CapsuleShortcuts *self,
                               GMenu            *menu)
{
  GObjectClass *klass;
  guint n_items;
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      if ((pspec = g_object_class_find_property (klass, id)) &&
          G_IS_PARAM_SPEC_STRING (pspec))
        {
          ...
          capsule_shortcuts_replace_key (menu, i, "accel", accel);
        }
    }
}
