...
G_DEFINE_FINAL_TYPE (CapsuleUser, capsule_user, G_TYPE_OBJECT)

static void
capsule_user_class_init (CapsuleUserClass *klass)
{
}

static void
capsule_user_init (CapsuleUser *self)
{
}

CapsuleUser *
capsule_user_get_default (void)
{
  static CapsuleUser *instance;

  if (instance == NULL)
    {
      instance = g_object_new (CAPSULE_TYPE_USER, NULL);
...
static void
capsule_user_discover_shell (GTask        *task,
                             ...
                             GCancellable *cancellable)
{
  ...
  g_assert (CAPSULE_IS_USER (source_object));
...
void
capsule_user_discover_shell_async (CapsuleUser         *self,
                                   GCancellable        *cancellable,
                                   ...
                                   gpointer             user_data)
{
  ...
  g_return_if_fail (CAPSULE_IS_USER (self));
  g_return_if_fail (!cancellable || G_IS_CANCELLABLE (cancellable));

  task = g_task_new (self, cancellable, callback, user_data);
  g_task_set_source_tag (task, capsule_user_discover_shell_async);

  switch (capsule_get_process_kind ())
    {
    case CAPSULE_PROCESS_KIND_HOST:
      g_task_return_pointer (task, egg_shell (g_getenv ("SHELL")), g_free);
      return;

    case CAPSULE_PROCESS_KIND_FLATPAK:
      g_task_run_in_thread (task, capsule_user_discover_shell);
      break;
...
char *
capsule_user_discover_shell_finish (CapsuleUser   *self,
                                    GAsyncResult  *result,
                                    GError       **error)
{
  g_return_val_if_fail (CAPSULE_IS_USER (self), NULL);
