...
#define G_LOG_DOMAIN "capsule-shortcut-accel-dialog"
...
struct _CapsuleShortcutAccelDialog
{
...
G_DEFINE_FINAL_TYPE (CapsuleShortcutAccelDialog, capsule_shortcut_accel_dialog, ADW_TYPE_WINDOW)

static gboolean
capsule_shortcut_accel_dialog_is_editing (CapsuleShortcutAccelDialog *self)
{
  g_assert (CAPSULE_IS_SHORTCUT_ACCEL_DIALOG (self));

  return self->editing;
}

static void
capsule_shortcut_accel_dialog_apply_state (CapsuleShortcutAccelDialog *self)
{
  g_assert (CAPSULE_IS_SHORTCUT_ACCEL_DIALOG (self));
                                       ...
                                       GtkEventControllerKey *controller)
{
  ...
  if (capsule_shortcut_accel_dialog_is_editing (self))
    {
      ...
      if (real_mask == 0 && keyval_lower == GDK_KEY_BackSpace)
        {
          capsule_shortcut_accel_dialog_set_accelerator (self, NULL);
      ...
      capsule_shortcut_accel_dialog_apply_state (self);
...
static void
shortcut_set_cb (CapsuleShortcutAccelDialog *self)
{
  g_signal_emit (self, signals [SHORTCUT_SET], 0,
                 capsule_shortcut_accel_dialog_get_accelerator (self));

  gtk_window_close (GTK_WINDOW (self));
}

static void
capsule_shortcut_accel_dialog_constructed (GObject *object)
{
  CapsuleShortcutAccelDialog *self = (CapsuleShortcutAccelDialog *)object;

  G_OBJECT_CLASS (capsule_shortcut_accel_dialog_parent_class)->constructed (object);

  gtk_widget_action_set_enabled (GTK_WIDGET (self), "shortcut.set", FALSE);
}

static void
capsule_shortcut_accel_dialog_finalize (GObject *object)
{
  CapsuleShortcutAccelDialog *self = (CapsuleShortcutAccelDialog *)object;

  g_clear_pointer (&self->shortcut_title, g_free);

  G_OBJECT_CLASS (capsule_shortcut_accel_dialog_parent_class)->finalize (object);
}
                                        ...
                                        GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_ACCELERATOR:
      g_value_take_string (value, capsule_shortcut_accel_dialog_get_accelerator (self));
      break;

    case PROP_SHORTCUT_TITLE:
      g_value_set_string (value, capsule_shortcut_accel_dialog_get_shortcut_title (self));
      break;
                                        ...
                                        GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_ACCELERATOR:
      capsule_shortcut_accel_dialog_set_accelerator (self, g_value_get_string (value));
      break;

    case PROP_SHORTCUT_TITLE:
      capsule_shortcut_accel_dialog_set_shortcut_title (self, g_value_get_string (value));
      break;
...
static void
capsule_shortcut_accel_dialog_class_init (CapsuleShortcutAccelDialogClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->constructed = capsule_shortcut_accel_dialog_constructed;
  object_class->finalize = capsule_shortcut_accel_dialog_finalize;
  object_class->get_property = capsule_shortcut_accel_dialog_get_property;
  object_class->set_property = capsule_shortcut_accel_dialog_set_property;
...
static void
capsule_shortcut_accel_dialog_init (CapsuleShortcutAccelDialog *self)
{
  self->editing = TRUE;
...
gchar *
capsule_shortcut_accel_dialog_get_accelerator (CapsuleShortcutAccelDialog *self)
{
  g_return_val_if_fail (CAPSULE_IS_SHORTCUT_ACCEL_DIALOG (self), NULL);
...
void
capsule_shortcut_accel_dialog_set_accelerator (CapsuleShortcutAccelDialog *self,
                                               const gchar                *accelerator)
{
  guint keyval;
  GdkModifierType state;

  g_return_if_fail (CAPSULE_IS_SHORTCUT_ACCEL_DIALOG (self));
...
void
capsule_shortcut_accel_dialog_set_shortcut_title (CapsuleShortcutAccelDialog *self,
...
const gchar *
capsule_shortcut_accel_dialog_get_shortcut_title (CapsuleShortcutAccelDialog *self)
{
  g_return_val_if_fail (CAPSULE_IS_SHORTCUT_ACCEL_DIALOG (self), NULL);
...
GtkWidget *
capsule_shortcut_accel_dialog_new (void)
{
  return g_object_new (CAPSULE_TYPE_SHORTCUT_ACCEL_DIALOG, NULL);
}
